...
class ConfigMaker:
	...
	def __init__(self, path=None, numfits=None, active_region=None):
		...
		self.active_region = np.random.randint(0,5, size=1) if active_region is None else active_region
		self.config['main'] = self.config_main
		self.config['star'] = self.config_star
		self.config['active_regions'] = self.config_active_regions
		
		'''self.check1 = check1 or 0
		self.check2 = check2 or 0
		self.check3 = check3 or 0
		self.check4 = check4 or 0'''
